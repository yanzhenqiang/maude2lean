-- Fraction type Frac = Int / (Nat+1)
-- Represented as (num : Int, den : Nat) for num/(den+1), denominator always positive
...
-- mk num den represents num/(den+1), so denominator should be (d1+1)*(d2+1) - 1
...
-- Fraction less-than: n1/d1 < n2/d2  iff  n1*d2 < n2*d1
